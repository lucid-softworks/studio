...
- [ ] Replace remaining whole-surface raster operations (notably full-layer filter evaluation) with sparse tiles or explicitly bounded jobs; selected-pixel clearing and content-aware-fill scanning, result transfer, commits, and history are selection-bounded, while plugin color matrices and atomic ICC conversion run off-thread against explicit asset bounds.
