...
- [ ] Replace the remaining Canvas2D whole-surface evaluation paths (notably advanced adjustments, Blend If, and document color output) with sparse tiles or explicit bounds; smart-filter graphs now evaluate only padded rotated-layer bounds with document-stable procedural coordinates, selected-pixel clearing and content-aware fill are selection-bounded, and plugin matrices plus atomic ICC conversion run off-thread.
