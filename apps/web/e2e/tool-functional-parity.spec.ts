...
test('magnetic lasso exposes edge controls and editable automatic anchors', async ({ page }) => {
  ...
  await expect(page.getByLabel('Magnetic lasso width')).toHaveValue('12')
  await expect(page.getByLabel('Magnetic lasso contrast')).toHaveValue('20')
  await expect(page.getByLabel('Magnetic lasso frequency')).toHaveValue('57')
  await page.getByLabel('Magnetic lasso width').fill('24')
  await page.getByLabel('Magnetic lasso contrast').fill('35')
  await page.getByLabel('Magnetic lasso frequency').fill('80')

  const surface = page.getByLabel('Magnetic lasso selection surface')
  const surfaceBounds = await surface.boundingBox()
  expect(surfaceBounds).not.toBeNull()
  await page.mouse.move(surfaceBounds!.x + surfaceBounds!.width * 0.28, surfaceBounds!.y + surfaceBounds!.height * 0.3)
  await page.mouse.down()
  await page.mouse.move(surfaceBounds!.x + surfaceBounds!.width * 0.7, surfaceBounds!.y + surfaceBounds!.height * 0.34, { steps: 8 })
  await page.mouse.move(surfaceBounds!.x + surfaceBounds!.width * 0.55, surfaceBounds!.y + surfaceBounds!.height * 0.68, { steps: 8 })
  await page.mouse.move(surfaceBounds!.x + surfaceBounds!.width * 0.28, surfaceBounds!.y + surfaceBounds!.height * 0.3, { steps: 8 })
  await page.mouse.up()
  await expect(page.getByRole('button', { name: 'Done', exact: true })).toBeVisible()
  const anchors = surface.getByLabel(/Magnetic anchor/)
  ...
  await page.mouse.down()
  await page.mouse.move(firstBounds!.x + 18, firstBounds!.y + 12, { steps: 3 })
  await page.mouse.up()
  await page.getByRole('button', { name: 'Delete last', exact: true }).click()
  await expect(anchors).toHaveCount(count - 1)
