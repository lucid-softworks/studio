...
test('layer menu and keyboard commands create, duplicate, delete, undo, and redo once', async ({ page }) => {
  ...
  await page.getByRole('menu', { name: 'Layer menu' }).getByRole('menuitem', { name: 'New layer', exact: true }).click()
  ...
  await expect(page.getByRole('button', { name: 'Layer 2 copy raster layer', exact: true })).toBeVisible()
...
test('groups duplicate and delete as complete nested stacks', async ({ page }) => {
  await page.getByRole('button', { name: 'Layer', exact: true }).click()
  await page.getByRole('menu', { name: 'Layer menu' }).getByRole('menuitem', { name: 'New group', exact: true }).click()
  await expect(page.getByText('1 object · 1 folder', { exact: true })).toBeVisible()

  await page.getByRole('button', { name: 'Layer', exact: true }).click()
  await page.getByRole('menu', { name: 'Layer menu' }).getByRole('menuitem', { name: 'Duplicate layer or group', exact: true }).click()
  ...
  await page.getByRole('button', { name: 'Layer', exact: true }).click()
  await page.getByRole('menu', { name: 'Layer menu' }).getByRole('menuitem', { name: 'Delete layer or group', exact: true }).click()
  await expect(page.getByText('1 object · 1 folder', { exact: true })).toBeVisible()
...
test('layer context menu operates on the row under the pointer', async ({ page }) => {
  await page.getByRole('button', { name: 'Layer 1 raster layer', exact: true }).click({ button: 'right' })
  ...
  await page.getByRole('button', { name: 'Layer 1 copy raster layer', exact: true }).click({ button: 'right' })
