...
function useAppController({ onExit, initialState, performanceMetrics, rendererOverride }: AppProps) {
  ...
  const activeRenderer = rendererOverride === 'canvas2d' ? 'canvas2d' : rendererCapabilities.activeRenderer
  ...
  useEffect(() => {
    if (canvasRef.current) canvasRef.current.dataset.renderer = activeRenderer
  }, [activeRenderer])
  ...
  const convertSelectedToSmartObject = () => {
    ...
    const contentLayer = {
      ...
      knockout: undefined,
      transparencyShapesLayer: undefined,
      blendInteriorEffectsAsGroup: undefined,
      channelBlendingRestrictions: undefined,
      effects: null,
      additionalEffects: [],
    } as EditorLayer
    ...
    const smartObject = {
      ...
      blendInteriorEffectsAsGroup: layer.blendInteriorEffectsAsGroup,
  ...
  }, [activeRenderer, assets, document, plugins.length, recoverySavedAt, rendererCapabilities.typegpu.state, saveStatus, setNotice])
      ...
      <header className="flex h-12 items-center justify-between border-b border-white/[0.07] bg-[#0e0e10] px-2.5 sm:px-3">
        ...
        <div className="hidden items-center gap-3 text-[9px] font-medium text-zinc-700 sm:flex">
          <span
            ...
            title={rendererCapabilities.typegpu.state === 'unavailable' || rendererCapabilities.typegpu.state === 'lost' ? rendererCapabilities.typegpu.reason : activeRenderer === 'webgpu' ? 'TypeGPU is compositing this document from native per-layer textures.' : rendererOverride === 'canvas2d' ? 'Canvas2D is forced for renderer comparison.' : 'This document uses composition features that currently require the Canvas2D compatibility renderer.'}
          >
            {activeRenderer === 'webgpu' ? 'TypeGPU · native layers' : `Canvas2D · ${rendererOverride === 'canvas2d' ? 'forced' : rendererCapabilities.typegpu.state === 'ready' ? 'compatibility' : rendererCapabilities.typegpu.state === 'initializing' ? 'Checking WebGPU…' : 'WebGPU fallback'}`}
          </span>
