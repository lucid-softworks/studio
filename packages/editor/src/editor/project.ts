...
function normalizeDocument(value: EditorDocument): EditorDocument {
  ...
  const countGroups = Array.isArray(value.counts?.groups) ? value.counts.groups.flatMap((group) => group && typeof group.id === 'string' ? [{ id: group.id, name: typeof group.name === 'string' && group.name.trim() ? group.name.trim().slice(0, 64) : 'Count group', color: typeof group.color === 'string' ? group.color : '#facc15' }] : []).slice(0, 32) : []
  if (!countGroups.length) countGroups.push({ id: 'count-group-1', name: 'Count group 1', color: '#facc15' })
  const countGroupIds = new Set(countGroups.map((group) => group.id))
  const countMarkers = Array.isArray(value.counts?.markers) ? value.counts.markers.flatMap((marker) => marker && typeof marker.id === 'string' && typeof marker.groupId === 'string' && countGroupIds.has(marker.groupId) && Number.isFinite(marker.x) && Number.isFinite(marker.y) ? [{ id: marker.id, groupId: marker.groupId, x: Math.max(0, Math.min(canvasSize.width, marker.x)), y: Math.max(0, Math.min(canvasSize.height, marker.y)), label: typeof marker.label === 'string' ? marker.label.slice(0, 80) : '' }] : []).slice(0, 10_000) : []
  const counts = { groups: countGroups, markers: countMarkers, activeGroupId: typeof value.counts?.activeGroupId === 'string' && countGroupIds.has(value.counts.activeGroupId) ? value.counts.activeGroupId : countGroups[0].id }
  const notes = Array.isArray(value.notes) ? value.notes.flatMap((note) => note && typeof note.id === 'string' && Number.isFinite(note.x) && Number.isFinite(note.y) ? [{ id: note.id, title: typeof note.title === 'string' && note.title.trim() ? note.title.slice(0, 80) : 'Note', content: typeof note.content === 'string' ? note.content.slice(0, 100_000) : '', author: typeof note.author === 'string' ? note.author.slice(0, 80) : '', color: typeof note.color === 'string' ? note.color : '#facc15', x: Math.max(0, Math.min(canvasSize.width, note.x)), y: Math.max(0, Math.min(canvasSize.height, note.y)), popupX: Number.isFinite(note.popupX) ? note.popupX : note.x + 24, popupY: Number.isFinite(note.popupY) ? note.popupY : note.y + 24, popupWidth: Math.max(120, Math.min(1200, Number(note.popupWidth) || 280)), popupHeight: Math.max(80, Math.min(900, Number(note.popupHeight) || 180)), open: note.open === true, date: typeof note.date === 'string' ? note.date.slice(0, 64) : '' }] : []).slice(0, 1000) : []
  let normalized = { ...value, bitDepth, canvasSize, groups, layers, selectedLayerId, selectedLayerIds, selectedGroupId, paths, selectedPathId, guides: Array.isArray(value.guides) ? value.guides : [], grid, artboards, colorMode, indexedColors: Math.max(2, Math.min(256, value.indexedColors ?? 256)), colorSettings, measurements, measurementScale, colorSamplers, counts, notes }
