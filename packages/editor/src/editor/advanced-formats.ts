...
async function decodeTiff(file: File, extension: string): Promise<AdvancedRasterImport> {
...
async function decodeHdr(file: File, extension: 'hdr' | 'exr'): Promise<AdvancedRasterImport> {
...
async function decodeHeif(file: File): Promise<AdvancedRasterImport> {
...
async function decodeAvif(file: File): Promise<AdvancedRasterImport> {
...
async function decodePdf(file: File): Promise<AdvancedRasterImport> {
...
async function decodeBrowserImage(file: File, format: 'ico'): Promise<AdvancedRasterImport> {
  const objectUrl = URL.createObjectURL(file)
  try {
    const element = await new Promise<HTMLImageElement>((resolve, reject) => { const image = new Image(); image.onload = () => resolve(image); image.onerror = () => reject(new Error('The browser could not decode this icon.')); image.src = objectUrl })
    const surface = document.createElement('canvas')
...
export async function importAdvancedRaster(file: File): Promise<AdvancedRasterImport> {
  const format = advancedFormatForFile(file)
  if (!format) throw new Error('No advanced local codec is registered for this file.')
  if (TIFF_EXTENSIONS.has(format)) return decodeTiff(file, format)
  if (format === 'hdr' || format === 'exr') return decodeHdr(file, format)
  if (format === 'heic' || format === 'heif') return decodeHeif(file)
  if (format === 'avif') return decodeAvif(file)
  if (format === 'pdf') return decodePdf(file)
  return decodeBrowserImage(file, 'ico')
}
