...
const geometryCornerIds = ['top-left', 'top-right', 'bottom-right', 'bottom-left'] as const
const transformMatrixLabels = ['A', 'B', 'C', 'D', 'E', 'F'] as const

function useInspectorController({ document, dispatch, endHistoryGroup, onBackgroundImage, backgroundImageName, customFonts, onLoadFont, onOpenSmartObject, onReplaceSmartObject, onRelinkSmartObject, onExportSmartObject, onContentAwareScale, canvasRef, renderer, onBitDepthChange, onChooseColorProfile, dockSide, onSwapPanels, width, onWidthChange, collapsed, onToggleCollapsed }: InspectorProps) {
  ...
  const geometry = normalizeGeometryTransform(selected?.geometryTransform)
  const updateGeometry = (patch: Partial<typeof geometry>, groupKey?: string) => {
    if (selected && selected.type !== 'adjustment') updateLayer(selected, { geometryTransform: { ...geometry, ...patch } }, groupKey)
  }
  ...
  return (
    <aside style={{ '--panel-width': `${width}px` } as CSSProperties} onDragOver={(event) => { if (event.dataTransfer.types.includes('application/x-studio-panel')) event.preventDefault() }} onDrop={(event) => { if (event.dataTransfer.getData('application/x-studio-panel') === 'layers') onSwapPanels() }} className={`relative order-2 flex w-full shrink-0 flex-col border-t border-white/[0.07] bg-[#111113] lg:h-[calc(100vh-84px)] lg:border-t-0 ${collapsed ? 'lg:w-10' : 'lg:w-[var(--panel-width)]'} ${dockSide === 'left' ? 'lg:order-1 lg:border-r' : 'lg:order-3 lg:border-l'}`}>
      ...
      <div draggable onDragStart={(event: DragEvent) => { event.dataTransfer.effectAllowed = 'move'; event.dataTransfer.setData('application/x-studio-panel', 'properties') }} className="flex h-14 shrink-0 cursor-grab items-center justify-between border-b border-white/[0.07] px-5 active:cursor-grabbing">
        <div>
          <h1 className="flex items-center gap-2 text-sm font-semibold text-zinc-100"><span className="text-[10px] tracking-tight text-zinc-700">⠿</span>Properties</h1>
      ...
      {selectedGroup ? (
        ...
        <>
              ...
              <>
                ...
                <div className="mt-4 flex items-center gap-2">
                  {document.background.gradient.map((color, index) => (
                    <label key={index === 0 ? 'start' : 'end'} className="flex flex-1 items-center gap-2 rounded-lg border border-white/[0.07] bg-white/[0.03] p-2 text-[10px] text-zinc-500">
                      <input
                        type="color"
                        value={color}
          ...
          <ControlSection title="Grid & guides">
            ...
            <div className="mt-3 grid grid-cols-3 gap-2">
              <label className="text-[9px] text-zinc-600">Spacing<input aria-label="Grid spacing" type="number" min="4" max="2000" value={document.grid?.spacing ?? 100} onChange={(event) => dispatch({ type: 'set-grid', patch: { spacing: numericInputValue(event.currentTarget, document.grid?.spacing ?? 100) } }, { groupKey: 'grid-spacing' })} onBlur={endHistoryGroup} className={`${fieldClass} mt-1 px-2`} /></label>
              <label className="text-[9px] text-zinc-600">Subdivisions<input aria-label="Grid subdivisions" type="number" min="1" max="10" value={document.grid?.subdivisions ?? 4} onChange={(event) => dispatch({ type: 'set-grid', patch: { subdivisions: numericInputValue(event.currentTarget, document.grid?.subdivisions ?? 4) } }, { groupKey: 'grid-subdivisions' })} onBlur={endHistoryGroup} className={`${fieldClass} mt-1 px-2`} /></label>
          ...
          <ControlSection title="Artboards">
            ...
            <div className="mt-3 space-y-2">
              {(document.artboards ?? []).map((artboard) => <div key={artboard.id} className="rounded-lg border border-white/[0.07] bg-black/20 p-2.5">
                ...
                <div className="mt-2 grid grid-cols-4 gap-1.5">{(['x', 'y', 'width', 'height'] as const).map((field) => <label key={field} className="text-[8px] text-zinc-700 uppercase">{field}<input aria-label={`${artboard.name} ${field}`} type="number" min={field === 'width' || field === 'height' ? 1 : 0} value={Math.round(artboard[field])} onChange={(event) => updateArtboard(artboard.id, { [field]: Math.max(field === 'width' || field === 'height' ? 1 : 0, numericInputValue(event.currentTarget, artboard[field])) })} className="mt-1 w-full rounded border border-white/[0.07] bg-zinc-950 px-1.5 py-1 font-mono text-[9px] text-zinc-500 outline-none" /></label>)}</div>
        ...
        <>
          ...
          {selected.type !== 'adjustment' && <ControlSection title="Transform geometry"><div className="grid grid-cols-2 gap-2"><label className="text-[9px] text-zinc-600">Interpolation<select aria-label="Transform interpolation" value={geometry.interpolation} onChange={(event) => updateGeometry({ interpolation: event.target.value as typeof geometry.interpolation })} className={`${fieldClass} mt-1`}><option value="nearest">Nearest</option><option value="bilinear">Bilinear</option><option value="bicubic">Bicubic</option></select></label><button type="button" onClick={() => updateLayer(selected, { geometryTransform: null })} className="mt-4 rounded-lg border border-white/[0.08] px-2 text-[9px] text-zinc-600 hover:text-zinc-200">Reset geometry</button></div><RangeControl label="Skew horizontal" value={geometry.skewX} min={-60} max={60} suffix="°" onChange={(skewX) => updateGeometry({ skewX }, `skew-x-${selected.id}`)} onChangeEnd={endHistoryGroup} /><RangeControl label="Skew vertical" value={geometry.skewY} min={-60} max={60} suffix="°" onChange={(skewY) => updateGeometry({ skewY }, `skew-y-${selected.id}`)} onChangeEnd={endHistoryGroup} /><RangeControl label="Perspective horizontal" value={geometry.perspectiveX} min={-100} max={100} suffix="%" onChange={(perspectiveX) => updateGeometry({ perspectiveX }, `perspective-x-${selected.id}`)} onChangeEnd={endHistoryGroup} /><RangeControl label="Perspective vertical" value={geometry.perspectiveY} min={-100} max={100} suffix="%" onChange={(perspectiveY) => updateGeometry({ perspectiveY }, `perspective-y-${selected.id}`)} onChangeEnd={endHistoryGroup} /><div className="mt-3"><p className="mb-2 text-[9px] text-zinc-600">Reference point</p><div className="mx-auto grid w-20 grid-cols-3 gap-1">{[0, 0.5, 1].flatMap((y) => [0, 0.5, 1].map((x) => <button key={`${x}:${y}`} type="button" aria-label={`Reference point ${x}, ${y}`} aria-pressed={geometry.referencePoint.x === x && geometry.referencePoint.y === y} onClick={() => updateGeometry({ referencePoint: { x, y } })} className={`aspect-square rounded-full border ${geometry.referencePoint.x === x && geometry.referencePoint.y === y ? 'border-violet-300 bg-violet-400/30' : 'border-white/[0.1] bg-black/20'}`} />))}</div></div><div className="mt-3 grid grid-cols-2 gap-2"><button type="button" onClick={() => updateGeometry({ warp: { columns: 3, rows: 3, points: Array.from({ length: 9 }, (_, index) => ({ x: index % 3 / 2, y: Math.floor(index / 3) / 2 })) } })} className="rounded-lg border border-white/[0.08] px-2 py-2 text-[9px] text-zinc-500 hover:text-zinc-200">Create 3×3 warp</button><button type="button" onClick={() => updateGeometry({ puppetPins: [...(geometry.puppetPins ?? []), { id: createId(), source: { x: 0.5, y: 0.5 }, position: { x: 0.5, y: 0.5 } }] })} className="rounded-lg border border-white/[0.08] px-2 py-2 text-[9px] text-zinc-500 hover:text-zinc-200">Add centre pin</button></div><div className="mt-3 grid grid-cols-4 gap-1">{geometryCornerIds.map((cornerId, index) => { const corner = geometry.corners[index]; return <button key={cornerId} type="button" title={`Reset corner ${index + 1}`} onClick={() => { const corners = structuredClone(geometry.corners); corners[index] = { x: 0, y: 0 }; updateGeometry({ corners }) }} className={`rounded-md py-2 text-[8px] ${corner.x || corner.y ? 'bg-cyan-400/10 text-cyan-200' : 'bg-white/[0.03] text-zinc-700'}`}>C{index + 1}</button> })}</div><p className="mt-2 text-[9px] leading-relaxed text-zinc-700">Use Warp or Puppet Warp in the toolbar to drag mesh points directly.</p></ControlSection>}
          ...
          {selected.type === 'raster' && (
            <ControlSection title="Raster layer">
              ...
              <div className="mt-4 border-t border-white/[0.06] pt-3"><p className="text-[9px] font-semibold tracking-[0.14em] text-zinc-600 uppercase">Content-aware scale</p><p className="mt-1 text-[9px] leading-relaxed text-zinc-700">Local seam carving protects high-contrast subjects without AI.</p><div className="mt-2 grid grid-cols-2 gap-2"><label className="text-[9px] text-zinc-600">Width %<input aria-label="Content-aware width percent" type="number" min="25" max="200" value={contentAwarePercent.width} onChange={(event) => setContentAwarePercent((current) => ({ ...current, width: numericInputValue(event.currentTarget, current.width) }))} className={`${fieldClass} mt-1`} /></label><label className="text-[9px] text-zinc-600">Height %<input aria-label="Content-aware height percent" type="number" min="25" max="200" value={contentAwarePercent.height} onChange={(event) => setContentAwarePercent((current) => ({ ...current, height: numericInputValue(event.currentTarget, current.height) }))} className={`${fieldClass} mt-1`} /></label></div><button type="button" onClick={() => onContentAwareScale(selected.id, selected.width * contentAwarePercent.width / 100, selected.height * contentAwarePercent.height / 100)} className="mt-2 w-full rounded-lg border border-cyan-300/15 bg-cyan-300/[0.04] px-3 py-2 text-[9px] text-cyan-100/70 hover:bg-cyan-300/[0.08]">Apply local seam carving</button></div>
            </ControlSection>
          )}
          ...
          {selected.type === 'smart-object' && (
            <ControlSection title="Smart object">
              ...
              <RangeControl label="Scale" value={selected.scale} min={10} max={300} suffix="%" onChange={(value) => updateLayer(selected, { scale: value }, `scale-${selected.id}`)} onChangeEnd={endHistoryGroup} />
              {selected.transformMatrix && <div className="mt-3"><p className="mb-2 text-[9px] font-semibold tracking-[0.14em] text-zinc-600 uppercase">Source matrix</p><div className="grid grid-cols-3 gap-1.5">{transformMatrixLabels.map((label, index) => <label key={label} className="block"><span className="sr-only">Matrix {label}</span><input type="number" step="0.01" value={Math.round(selected.transformMatrix![index] * 1000) / 1000} onChange={(event) => { const transformMatrix = [...selected.transformMatrix!] as [number, number, number, number, number, number]; transformMatrix[index] = numericInputValue(event.currentTarget, transformMatrix[index]); updateLayer(selected, { transformMatrix }, `matrix-${selected.id}-${index}`) }} onBlur={endHistoryGroup} className="w-full rounded-md border border-white/[0.07] bg-black/25 px-2 py-1.5 font-mono text-[9px] text-zinc-400 outline-none focus:border-cyan-300/40" /></label>)}</div></div>}
...
export function Inspector(props: InspectorProps) {
  return useInspectorController(props)
}
