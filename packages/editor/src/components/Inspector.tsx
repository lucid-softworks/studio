        ...
        <>
          ...
          {selected.type === 'smart-object' && (
            <ControlSection title="Smart object">
              ...
              <RangeControl label="Scale" value={selected.scale} min={10} max={300} suffix="%" onChange={(value) => updateLayer(selected, { scale: value }, `scale-${selected.id}`)} onChangeEnd={endHistoryGroup} />
              {selected.transformMatrix && <div className="mt-3"><p className="mb-2 text-[9px] font-semibold tracking-[0.14em] text-zinc-600 uppercase">Source matrix</p><div className="grid grid-cols-3 gap-1.5">{transformMatrixLabels.map((label, index) => <label key={label} className="block"><span className="sr-only">Matrix {label}</span><input type="number" step="0.01" value={Math.round(selected.transformMatrix![index] * 1000) / 1000} onChange={(event) => { const transformMatrix = [...selected.transformMatrix!] as [number, number, number, number, number, number]; transformMatrix[index] = Number(event.target.value); updateLayer(selected, { transformMatrix }, `matrix-${selected.id}-${index}`) }} onBlur={endHistoryGroup} className="w-full rounded-md border border-white/[0.07] bg-black/25 px-2 py-1.5 font-mono text-[9px] text-zinc-400 outline-none focus:border-cyan-300/40" /></label>)}</div></div>}
