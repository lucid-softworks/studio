...
export function CurvesControl({ curves, canvasRef, onChange, onChangeEnd }: Props) {
  ...
  const [histogram, setHistogram] = useState<number[]>(() => Array.from({ length: 256 }, () => 0))
  const [dragIndex, setDragIndex] = useState<number | null>(null)
  const svgRef = useRef<SVGSVGElement>(null)
  const points = useMemo(() => [...(curves[channel] ?? curvePreset('linear'))].sort((left, right) => left.input - right.input), [channel, curves])

  useEffect(() => {
    const canvas = canvasRef.current
    const context = canvas?.getContext('2d', { willReadFrequently: true })
    if (!canvas || !context) return
    try { setHistogram(visibleHistogram(context.getImageData(0, 0, canvas.width, canvas.height).data, channel)) } catch { setHistogram(Array.from({ length: 256 }, () => 0)) }
  }, [canvasRef, channel, curves])
  ...
  return <div className="rounded-lg border border-white/[0.07] bg-black/20 p-2.5">
    ...
    <svg ref={svgRef} viewBox="0 0 255 255" aria-label={`${channel} curve editor`} className="aspect-square w-full touch-none rounded border border-white/[0.08] bg-[#0a0a0c]" onPointerDown={(event) => { const next = curvePoint(event); const updated = [...points, next].sort((left, right) => left.input - right.input); setDragIndex(updated.findIndex((point) => point === next)); onChange(channel, updated, `curve-${channel}`); event.currentTarget.setPointerCapture(event.pointerId) }} onPointerMove={(event) => { if (dragIndex !== null) setPoint(dragIndex, curvePoint(event)) }} onPointerUp={() => { setDragIndex(null); onChangeEnd() }} onPointerCancel={() => { setDragIndex(null); onChangeEnd() }}>
      ...
      {points.map((point, index) => <circle key={`${point.input}-${index}`} cx={point.input} cy={255 - point.output} r="4" fill="#18181b" stroke={channelColor[channel]} strokeWidth="2" onPointerDown={(event) => { event.stopPropagation(); setDragIndex(index); event.currentTarget.ownerSVGElement?.setPointerCapture(event.pointerId) }} onDoubleClick={(event) => { event.stopPropagation(); if (index > 0 && index < points.length - 1) onChange(channel, points.filter((_, candidate) => candidate !== index)) }} />)}
    </svg>
