...
export function Toast({ value, onDismiss }: { value: ToastMessage; onDismiss: () => void }) {
  ...
  return (
    <div role={value.tone === 'error' ? 'alert' : 'status'} className={`fixed top-16 right-4 z-50 flex w-[min(28rem,calc(100vw-2rem))] items-start gap-3 rounded-lg border px-3.5 py-3 text-xs shadow-[0_18px_55px_rgba(0,0,0,0.45)] backdrop-blur-xl ${style.shell}`}>
      <span className={`mt-1 size-2 shrink-0 rounded-full shadow-[0_0_12px_currentColor] ${style.dot}`} />
      <div className="min-w-0 flex-1"><p className="text-[9px] font-bold tracking-[0.14em] opacity-55 uppercase">{style.label}</p><p className="mt-0.5 max-h-64 overflow-y-auto pr-1 leading-relaxed whitespace-pre-line">{value.message}</p></div>
