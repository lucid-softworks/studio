...
import { CloneStampOverlay, type CloneSource } from './CloneStampOverlay'
...
const cloneSourceSlotIds = ['clone-source-1', 'clone-source-2', 'clone-source-3', 'clone-source-4', 'clone-source-5'] as const
...
function hintForTool(tool: EditorTool, context: { hasCrop: boolean; hasSelection: boolean; editingMaskName?: string; rasterSelected: boolean }) {
  switch (tool) {
    ...
    case 'note': return 'Click the canvas to place a persistent text annotation'
...
function useCanvasStageController({ canvasRef, document, assets, dispatch, endHistoryGroup, isLoading, onFile, canUndo, canRedo, onUndo, onRedo, onAlign, onRasterChange, onRasterCommit, editingMaskLayerId, selection, onSelectionChange: setSelection, zoom, onZoomChange: setZoom, tool, shortcuts, onToolChange, onAddText, onAddShape, onCrop, onPerspectiveCrop, brushes, brushId, onBrushChange, onLoadBrush, foregroundColor, backgroundColor, gradientStops, onForegroundColorChange, onBackgroundColorChange }: CanvasStageProps) {
  ...
  const [cloneSources, setCloneSources] = useState<Array<CloneSource | null>>(() => Array.from({ length: 5 }, () => null))
  const [activeCloneSource, setActiveCloneSource] = useState(0)
  const [cloneOverlayVisible, setCloneOverlayVisible] = useState(true)
  const [cloneOverlayOpacity, setCloneOverlayOpacity] = useState(50)
  const [cloneOverlayClipped, setCloneOverlayClipped] = useState(true)
  const [cloneOverlayInverted, setCloneOverlayInverted] = useState(false)
  const [tolerance, setTolerance] = useState(32)
  const [selectionMode, setSelectionMode] = useState<SelectionMode>('replace')
  const [magneticWidth, setMagneticWidth] = useState(12)
  const [magneticContrast, setMagneticContrast] = useState(20)
  const [magneticFrequency, setMagneticFrequency] = useState(57)
  const [quickMask, setQuickMask] = useState(false)
  const [cropDraft, setCropDraft] = useState<{ canvasKey: string; selection: SelectionState | null }>({ canvasKey: '', selection: null })
  ...
  const cloneSource = cloneSources[activeCloneSource]
  ...
  const cropCanvasKey = `${preset.width}x${preset.height}`
  const cropSelection = cropDraft.canvasKey === cropCanvasKey && tool === 'crop' ? cropDraft.selection : null
  const setCropSelection = (next: SelectionState | null) => setCropDraft({ canvasKey: cropCanvasKey, selection: next })
  ...
  const patchCloneSource = (patch: Partial<CloneSource>) => {
    setCloneSources((current) => current.map((source, index) => index === activeCloneSource && source ? { ...source, ...patch } : source))
  }

  const setCloneSource = (source: CloneSource) => {
    setCloneSources((current) => current.map((candidate, index) => index === activeCloneSource ? source : candidate))
  }
  ...
  const addNote = (position: Position) => {
    ...
    dispatch({ type: 'set-notes', notes: [...notes, { id: createId(), title: `Note ${notes.length + 1}`, content: '', author: '', color: '#facc15', x: position.x, y: position.y, popupX: position.x + 24, popupY: position.y + 24, popupWidth: 280, popupHeight: 180, open: true, date: new Date().toISOString() }].slice(-1000) })
  ...
  return (
    ...
    >
      <div className="flex h-14 shrink-0 items-center justify-between border-b border-white/[0.06] px-4 sm:px-5">
        ...
        <div className="flex min-w-0 items-center gap-3">
          {(paintTool || retouchTool || pixelRetouchTool) && (
            <div className="studio-tab-strip hidden min-w-0 max-w-full items-center gap-2 overflow-x-auto md:flex">
              ...
              <input aria-label="Brush size" type="range" min="2" max="240" value={brushSize} onChange={(event) => setBrushSize(numericInputValue(event.currentTarget, brushSize))} className="studio-range w-20" />
              ...
              {(tool === 'dodge' || tool === 'burn' || tool === 'sponge') && <><span className="text-[9px] text-zinc-600">Flow</span><input aria-label="Tone tool flow" type="range" min="1" max="100" value={brushFlow} onChange={(event) => setBrushFlow(numericInputValue(event.currentTarget, brushFlow))} className="studio-range w-14" /><span className="w-7 font-mono text-[9px] text-zinc-600">{brushFlow}%</span></>}
              {retouchTool && <div className="hidden items-center gap-1 xl:flex"><div className="flex rounded border border-white/[0.08] bg-black/20 p-0.5">{cloneSourceSlotIds.map((id, index) => { const source = cloneSources[index]; return <button key={id} type="button" aria-label={`Clone source ${index + 1}`} aria-pressed={activeCloneSource === index} title={source ? `Source ${index + 1} set` : `Source ${index + 1} empty`} onClick={() => setActiveCloneSource(index)} className={`size-5 rounded text-[8px] ${activeCloneSource === index ? 'bg-violet-400/20 text-violet-200' : source ? 'text-zinc-300' : 'text-zinc-700'}`}>{index + 1}</button> })}</div><select aria-label="Clone sample mode" value={cloneSampleMode} onChange={(event) => setCloneSampleMode(event.target.value as typeof cloneSampleMode)} className="rounded-md border border-white/[0.08] bg-black/25 px-1.5 py-1 text-[9px] text-zinc-400"><option value="current">Current</option><option value="current-and-below">Current &amp; below</option></select><label className="flex items-center gap-1 text-[9px] text-zinc-600"><input type="checkbox" checked={cloneAligned} onChange={(event) => setCloneAligned(event.target.checked)} />Aligned</label><label className="flex items-center gap-1 text-[9px] text-zinc-600">X<input aria-label="Clone source horizontal offset" type="number" disabled={!cloneSource} value={cloneSource?.offsetX ?? 0} onChange={(event) => patchCloneSource({ offsetX: numericInputValue(event.currentTarget, cloneSource?.offsetX ?? 0) })} className="w-11 rounded border border-white/[0.08] bg-black/25 px-1 py-1 font-mono" /></label><label className="flex items-center gap-1 text-[9px] text-zinc-600">Y<input aria-label="Clone source vertical offset" type="number" disabled={!cloneSource} value={cloneSource?.offsetY ?? 0} onChange={(event) => patchCloneSource({ offsetY: numericInputValue(event.currentTarget, cloneSource?.offsetY ?? 0) })} className="w-11 rounded border border-white/[0.08] bg-black/25 px-1 py-1 font-mono" /></label><label className="flex items-center gap-1 text-[9px] text-zinc-600">∠<input aria-label="Clone source rotation" type="number" min="-180" max="180" disabled={!cloneSource} value={cloneSource?.rotation ?? 0} onChange={(event) => patchCloneSource({ rotation: Math.max(-180, Math.min(180, numericInputValue(event.currentTarget, cloneSource?.rotation ?? 0))) })} className="w-11 rounded border border-white/[0.08] bg-black/25 px-1 py-1 font-mono" /></label><label className="flex items-center gap-1 text-[9px] text-zinc-600">%<input aria-label="Clone source scale" type="number" min="10" max="400" disabled={!cloneSource} value={cloneSource?.scale ?? 100} onChange={(event) => patchCloneSource({ scale: Math.max(10, Math.min(400, numericInputValue(event.currentTarget, cloneSource?.scale ?? 100))) })} className="w-11 rounded border border-white/[0.08] bg-black/25 px-1 py-1 font-mono" /></label><button type="button" aria-label="Flip clone source horizontally" aria-pressed={cloneSource?.flipX ?? false} disabled={!cloneSource} onClick={() => patchCloneSource({ flipX: !cloneSource?.flipX })} className="rounded px-1 text-[9px] text-zinc-500 aria-pressed:bg-violet-400/15 aria-pressed:text-violet-200">↔</button><button type="button" aria-label="Flip clone source vertically" aria-pressed={cloneSource?.flipY ?? false} disabled={!cloneSource} onClick={() => patchCloneSource({ flipY: !cloneSource?.flipY })} className="rounded px-1 text-[9px] text-zinc-500 aria-pressed:bg-violet-400/15 aria-pressed:text-violet-200">↕</button><label className="flex items-center gap-1 text-[9px] text-zinc-600"><input aria-label="Show clone source overlay" type="checkbox" checked={cloneOverlayVisible} onChange={(event) => setCloneOverlayVisible(event.target.checked)} />Overlay</label><input aria-label="Clone source overlay opacity" title="Overlay opacity" type="range" min="5" max="100" value={cloneOverlayOpacity} onChange={(event) => setCloneOverlayOpacity(numericInputValue(event.currentTarget, cloneOverlayOpacity))} className="studio-range w-12" /><label className="flex items-center gap-1 text-[9px] text-zinc-600"><input aria-label="Clip clone source overlay" type="checkbox" checked={cloneOverlayClipped} onChange={(event) => setCloneOverlayClipped(event.target.checked)} />Clip</label><label className="flex items-center gap-1 text-[9px] text-zinc-600"><input aria-label="Invert clone source overlay" type="checkbox" checked={cloneOverlayInverted} onChange={(event) => setCloneOverlayInverted(event.target.checked)} />Invert</label></div>}
            </div>
          )}
          ...
          {(tool === 'magic-wand' || tool === 'fill') && <label className="hidden items-center gap-2 text-[9px] text-zinc-600 xl:flex"><span>Tolerance</span><input aria-label="Tolerance" type="range" min="0" max="128" value={tolerance} onChange={(event) => setTolerance(numericInputValue(event.currentTarget, tolerance))} className="studio-range w-16" /><span className="w-5 font-mono">{tolerance}</span></label>}
          {tool === 'magnetic-lasso' && <div className="hidden items-center gap-2 text-[9px] text-zinc-600 lg:flex"><label className="flex items-center gap-1">Width<input aria-label="Magnetic lasso width" type="number" min="3" max="256" value={magneticWidth} onChange={(event) => setMagneticWidth(Math.max(3, Math.min(256, numericInputValue(event.currentTarget, magneticWidth))))} className="w-12 rounded border border-white/[0.08] bg-black/25 px-1 py-1 font-mono" />px</label><label className="flex items-center gap-1">Contrast<input aria-label="Magnetic lasso contrast" type="range" min="1" max="100" value={magneticContrast} onChange={(event) => setMagneticContrast(numericInputValue(event.currentTarget, magneticContrast))} className="studio-range w-14" /><span className="w-5 font-mono">{magneticContrast}</span></label><label className="flex items-center gap-1">Frequency<input aria-label="Magnetic lasso frequency" type="range" min="1" max="100" value={magneticFrequency} onChange={(event) => setMagneticFrequency(numericInputValue(event.currentTarget, magneticFrequency))} className="studio-range w-14" /><span className="w-5 font-mono">{magneticFrequency}</span></label></div>}
          {selectionTool && (
      ...
      >
        ...
        <div className={`relative z-10 grid w-full max-w-full shrink-0 items-center gap-3 ${splitView ? 'grid-cols-2' : 'grid-cols-1'}`}>
          <div className="flex min-w-0 items-center justify-center transition-transform duration-150" style={{ aspectRatio: `${preset.width} / ${preset.height}`, transform: `scale(${zoom / 100}) rotate(${viewRotation}deg)` }}>
            ...
            <LassoSelectionOverlay canvasRef={canvasRef} enabled={tool === 'lasso' || tool === 'magnetic-lasso'} magnetic={tool === 'magnetic-lasso'} magneticWidth={magneticWidth} magneticContrast={magneticContrast} magneticFrequency={magneticFrequency} mode={selectionMode} selection={selection} onChange={setSelection} />
            ...
            {(tool === 'magic-wand' || tool === 'object-select') && <MagicWandOverlay canvasRef={canvasRef} object={tool === 'object-select'} mode={selectionMode} tolerance={tolerance} selection={selection} onChange={setSelection} />}
            ...
            {retouchTool && <CloneStampOverlay canvasRef={canvasRef} document={document} assets={assets} tool={tool} size={brushSize} strength={toolStrength} aligned={cloneAligned} sampleMode={cloneSampleMode} source={cloneSource} onSourceChange={setCloneSource} overlayVisible={cloneOverlayVisible} overlayOpacity={cloneOverlayOpacity} overlayClipped={cloneOverlayClipped} overlayInverted={cloneOverlayInverted} selection={selection} locked={selectedLocked} onChange={onRasterChange} onCommit={onRasterCommit} />}
            ...
            <CanvasActionOverlay canvasRef={canvasRef} tool={tool} samplers={document.colorSamplers} counts={counts} notes={document.notes} onColorSample={sampleColor} onAddCount={addCount} onAddNote={addNote} onAddText={(position, paragraphBox) => onAddText(position, foregroundColor, paragraphBox)} onAddShape={(shape, position) => onAddShape(shape, position, foregroundColor)} onZoom={changeZoom} />
            </div>
          </div>
      ...
      {notesDialogOpen && <NotesDialog notes={document.notes ?? []} onChange={(notes) => dispatch({ type: 'set-notes', notes }, { groupKey: 'notes' })} onClose={() => { endHistoryGroup(); setNotesDialogOpen(false) }} />}
    </section>
  )
}
