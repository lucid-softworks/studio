    ...
    >
      <div className="flex h-14 shrink-0 items-center justify-between border-b border-white/[0.06] px-4 sm:px-5">
        ...
        <div className="flex min-w-0 items-center gap-3">
          ...
          {tool === 'magnetic-lasso' && <div className="hidden items-center gap-2 text-[9px] text-zinc-600 lg:flex"><label className="flex items-center gap-1">Width<input aria-label="Magnetic lasso width" type="number" min="3" max="256" value={magneticWidth} onChange={(event) => setMagneticWidth(Math.max(3, Math.min(256, Number(event.target.value))))} className="w-12 rounded border border-white/[0.08] bg-black/25 px-1 py-1 font-mono" />px</label><label className="flex items-center gap-1">Contrast<input aria-label="Magnetic lasso contrast" type="range" min="1" max="100" value={magneticContrast} onChange={(event) => setMagneticContrast(Number(event.target.value))} className="studio-range w-14" /><span className="w-5 font-mono">{magneticContrast}</span></label><label className="flex items-center gap-1">Frequency<input aria-label="Magnetic lasso frequency" type="range" min="1" max="100" value={magneticFrequency} onChange={(event) => setMagneticFrequency(Number(event.target.value))} className="studio-range w-14" /><span className="w-5 font-mono">{magneticFrequency}</span></label></div>}
          {selectionTool && (
