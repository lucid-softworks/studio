  ...
  return (
    <div role="tabpanel" aria-label="Info" className="min-h-0 flex-1 overflow-y-auto p-3">
      ...
      <section className="mt-3 rounded-lg border border-white/[0.07] bg-black/15 p-3">
        ...
        <div className="mt-3 grid grid-cols-3 gap-2"><label className="text-[8px] text-zinc-600">X<input aria-label="Sample x coordinate" type="number" min="0" max={size.width - 1} value={sampleX} onChange={(event) => setSampleX(numericInputValue(event.currentTarget, sampleX))} className="mt-1 w-full rounded border border-white/[0.07] bg-zinc-950 px-2 py-1.5 font-mono text-[9px] text-zinc-300" /></label><label className="text-[8px] text-zinc-600">Y<input aria-label="Sample y coordinate" type="number" min="0" max={size.height - 1} value={sampleY} onChange={(event) => setSampleY(numericInputValue(event.currentTarget, sampleY))} className="mt-1 w-full rounded border border-white/[0.07] bg-zinc-950 px-2 py-1.5 font-mono text-[9px] text-zinc-300" /></label><label className="text-[8px] text-zinc-600">Average<select aria-label="Point sample size" value={sampleSize} onChange={(event) => setSampleSize(Number(event.target.value) as 1 | 3 | 5)} className="mt-1 w-full rounded border border-white/[0.07] bg-zinc-950 px-2 py-1.5 font-mono text-[9px] text-zinc-300"><option value="1">1 × 1</option><option value="3">3 × 3</option><option value="5">5 × 5</option></select></label></div>
