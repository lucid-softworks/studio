...
function loadScripts(): SavedScript[] {
  try {
    const value = JSON.parse(localStorage.getItem('studio.local-scripts') ?? '[]') as unknown
...
export function ScriptSandboxDialog({ document, onClose }: { document: EditorDocument; onClose: () => void }) {
  ...
  const [inputFiles, setInputFiles] = useState<ScriptFile[]>([])
  const [output, setOutput] = useState<string[]>([])
  const [running, setRunning] = useState(false)
  const [sandboxReady, setSandboxReady] = useState(false)

  useEffect(() => { try { localStorage.setItem('studio.local-scripts', JSON.stringify(scripts)) } catch { /* Script storage is optional. */ } }, [scripts])
  useEffect(() => {
  ...
  const run = async () => {
    ...
    const result = await new Promise<SandboxResult>((resolve) => {
      ...
      target.postMessage({ token, code, context: { document: structuredClone(document), inputFiles: allowRead ? inputFiles : [] } }, '*')
    })
    setRunning(false)
