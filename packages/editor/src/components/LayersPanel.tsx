...
import { ChannelsPanel, GradientsPanel, HistogramPanel, HistoryPanel, InfoPanel, LibrariesPanel, NavigatorPanel, PathsPanel, PatternsPanel, PluginPanelsPanel, SwatchesPanel, type AlphaChannelTransform } from './UtilityPanels'
...
const utilityTabs: Array<{ id: UtilityPanelId; label: string; title?: string }> = [{ id: 'layers', label: 'Layers' }, { id: 'channels', label: 'Chan', title: 'Channels' }, { id: 'paths', label: 'Paths' }, { id: 'history', label: 'History' }, { id: 'actions', label: 'Act', title: 'Actions' }, { id: 'navigator', label: 'Nav', title: 'Navigator' }, { id: 'histogram', label: 'Hist', title: 'Histogram' }, { id: 'swatches', label: 'Swat', title: 'Swatches' }, { id: 'gradients', label: 'Grad', title: 'Gradients' }, { id: 'patterns', label: 'Patt', title: 'Patterns' }, { id: 'libraries', label: 'Lib', title: 'Libraries' }, { id: 'plugins', label: 'Plug', title: 'Plugin panels' }, { id: 'info', label: 'Info' }]
...
export function LayersPanel({ document, dispatch, onAddLayer, onAddAdjustment, onAddGroup, onDuplicateSelection, editingMaskLayerId, onAddMask, onEditMask, onRemoveMask, dockSide, onSwapPanels, width, onWidthChange, collapsed, onToggleCollapsed, activePanel, onActivePanelChange, assets, canvasRef, selection, onLoadComponentChannel, onSaveAlphaChannel, onLoadAlphaChannel, onDuplicateAlphaChannel, onDeleteAlphaChannel, onTransformAlphaChannel, onFillPath, onStrokePath, customShapes, onSaveCustomShape, onApplyCustomShape, onRemoveCustomShape, onImportCustomShape, onExportPath, zoom, onZoomChange, renderer, historyPast, historyFuture, rasterUndoDepth, onJumpHistory, renderRevision, panelOrder, onPanelOrderChange, floating, floatingPosition, onFloatingPositionChange, onToggleFloating, secondaryPanel, onSecondaryPanelChange, secondaryHeight, onSecondaryHeightChange, secondaryFloating, onToggleSecondaryFloating, secondaryFloatingPosition, onSecondaryFloatingPositionChange, onRunActions, plugins, foregroundColor, backgroundColor, customSwatches, onForegroundColorChange, onBackgroundColorChange, onAddSwatch, onRemoveSwatch, customGradients, onApplyGradient, onAddGradient, onRemoveGradient, customPatterns, onApplyPattern, onAddPattern, onRemovePattern, onImportPattern, onExportPattern, brushes, brushId, customFonts, onBrushChange, onLoadBrush, onRemoveBrush, onExportBrush, onLoadFont, onRemoveFont }: LayersPanelProps) {
  ...
  const layerRow = (layer: EditorLayer, depth: number) => {
    ...
    return (
      ...
      >
        <button type="button" aria-label={`${layer.name} ${layer.type} layer`} onClick={(event) => dispatch({ type: 'select-layer', id: layer.id, mode: event.shiftKey || event.metaKey || event.ctrlKey ? 'toggle' : 'replace' }, { record: false })} className="flex min-w-0 flex-1 items-center gap-2 rounded-md p-1 text-left focus-visible:outline-2 focus-visible:outline-violet-400">
  ...
  const utilityPanelContent = (panel: UtilityPanelId) => {
    ...
    if (panel === 'plugins') return <PluginPanelsPanel plugins={plugins} />
  ...
  return (
    ...
    {contextMenu && <div role="menu" aria-label="Layer context menu" style={{ left: contextMenu.x, top: contextMenu.y }} onPointerDown={(event) => event.stopPropagation()} className="fixed z-[90] w-48 rounded-xl border border-white/[0.1] bg-[#18181b]/98 p-1.5 shadow-[0_24px_70px_rgba(0,0,0,0.65)] backdrop-blur-xl">
      ...
      <hr className="my-1 border-t border-white/[0.07]" />
    ...
    <aside aria-label="Utility panel stack" style={panelStyle} onDragOver={(event) => { if (event.dataTransfer.types.includes('application/x-studio-panel')) event.preventDefault() }} onDrop={(event) => { if (event.dataTransfer.getData('application/x-studio-panel') === 'properties') onSwapPanels() }} className={floating ? 'fixed z-[65] flex h-[min(72vh,680px)] w-[var(--panel-width)] shrink-0 flex-col overflow-hidden rounded-xl border border-white/[0.12] bg-[#111113] shadow-[0_24px_80px_rgba(0,0,0,0.7)]' : `relative order-3 flex w-full shrink-0 flex-col border-t border-white/[0.07] bg-[#111113] lg:h-[calc(100vh-84px)] lg:border-t-0 ${panelCollapsed ? 'lg:w-10' : 'lg:w-[var(--panel-width)]'} ${dockSide === 'left' ? 'lg:order-1 lg:border-r' : 'lg:order-3 lg:border-l'}`}>
      ...
      <div draggable={!floating} onDragStart={(event) => { event.dataTransfer.effectAllowed = 'move'; event.dataTransfer.setData('application/x-studio-panel', 'layers') }} className="flex h-10 shrink-0 cursor-grab items-center border-b border-white/[0.07] px-1.5 active:cursor-grabbing">
        {floating && <button type="button" aria-label="Move floating panels" title="Drag floating panels" onPointerDown={startFloatingDrag} className="mr-1 flex size-7 shrink-0 touch-none items-center justify-center rounded text-[10px] tracking-tight text-zinc-700 hover:bg-white/[0.05] hover:text-zinc-300">⠿</button>}
        <div role="tablist" aria-label="Utility panels" className="studio-tab-strip flex min-w-0 flex-1 items-center overflow-x-auto">
      ...
      {!activeGroup && document.selectedLayerIds.length > 0 && <div className="flex items-center justify-between border-t border-white/[0.07] p-3"><div className="flex items-center gap-1">{document.selectedLayerIds.length === 1 && activeLayer && <><button type="button" onClick={() => activeLayer.maskAssetId ? onEditMask(activeLayer.id) : onAddMask(activeLayer.id)} className={`rounded-md border px-2 py-1 text-[9px] ${editingMaskLayerId === activeLayer.id ? 'border-cyan-300/20 bg-cyan-400/10 text-cyan-200' : 'border-white/[0.07] text-zinc-600'}`}>{activeLayer.maskAssetId ? editingMaskLayerId === activeLayer.id ? 'Pixels' : 'Mask' : '+ Mask'}</button>{activeLayer.maskAssetId && <button type="button" aria-label="Remove layer mask" onClick={() => onRemoveMask(activeLayer.id)} className="flex size-6 items-center justify-center rounded text-zinc-700 hover:text-red-300">×</button>}</>}{document.selectedLayerIds.length > 1 && <p className="text-[10px] text-zinc-700">{document.selectedLayerIds.length} selected</p>}</div><button type="button" aria-label="Delete selected layer" onClick={() => dispatch({ type: 'remove-layers', ids: document.selectedLayerIds })} className="flex size-7 items-center justify-center rounded-md text-zinc-600 hover:bg-red-400/10 hover:text-red-300"><TrashIcon className="size-3.5" /></button></div>}
      </>}
      ...
      {activePanel === 'plugins' && <PluginPanelsPanel plugins={plugins} />}
